...
router.post('/', async (req, res) => {
    try {
        let { name, email, message } = req.body; // Destructure directly

        let createMessage = await controller.sendMessage(name, email, message); // Pass arguments separately
        if (createMessage.error === false) {
            ...
            res.status(400).json({ error: true, message: 'Error sending message from backend to frontend' });
        }
    } catch (error) {
